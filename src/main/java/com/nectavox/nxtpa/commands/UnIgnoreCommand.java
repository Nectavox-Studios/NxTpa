...
@Command(name = "nxtpunignore")
@HasPermission(Perms.UNIGNORE_CMD)
@PlayerOnly
public class UnIgnoreCommand extends CommandBase {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            IgnoreGUI.openIgnoredPlayers(player, plugin);
        } else {
            ...
            plugin.getDataManager().unIgnorePlayer(player, target);
        }
    }

    @Completer("nxtpunignore")
